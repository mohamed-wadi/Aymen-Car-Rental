    ...
    function adjustHeader()
    {
        ...
        if(windowWidth > 0) {
            if ($(document).scrollTop() >= 100) {
                ...
                // Changer vers le logo noir quand on scroll (navbar devient blanche)
                $('.company-logo img').attr('src', '/static/img/logos/black-logo.png');
                $('.company-logo img').attr('width', '210');
                $('.company-logo img').attr('height', '35');
            }
            else {
                $('.sticky-header').removeClass('header-shrink');
                // Revenir au logo blanc original quand on remonte (navbar redevient sombre)
                if($('.fixed-header').length == 0 && $('.fixed-header2').length == 0) {
